        ...
        static void Main()
        {
            ...
            using (_connection = _factory.CreateConnection())
            {
                using (var channel = _connection.CreateModel())
                {
                    ...
                    // telling applciation we are bound exchange and listnend to 'purchase order' messages
                    channel.QueueBind(PurchaseOrderQueueName, ExchangeName, "PurchaseOrder");
                    // not more than one message to worker till it is consumed else give it to next worker
